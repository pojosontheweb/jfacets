...
CodePress = {
	...
	edit : function() {
		...
		if(!arguments[1]||arguments[1]=='') { // file name of the source code (to open from server)
			this.setLanguage();
			cpEditor.src = cpPath+'modules/codepress.jsp?action=edit&file='+this.fileName+'&language='+this.language+'&engine='+cpEngine
			return;
		}
	...
	setContent : function() {
		...
		$('codepress').innerHTML = '<div id="cp-window">'+
			'<iframe id="cp-editor" src="'+cpPath+'modules/codepress.jsp?engine='+cpEngine+'&file='+ (onLoadEdit ? 'null' : this.fileName) +'&language='+this.language+'" style="height:'+ cpEditorHeight +'px"></iframe>'+
			'<div id="cp-menu">'+
